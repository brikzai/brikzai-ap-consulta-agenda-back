...
15. **`consultar_agenda` pode deixar `consulta_agenda` presa em `PARCIAL` para sempre em falhas fora do catálogo CERC (achado na re-revisão da correção final do Plano 06).** `_criar_consulta_agenda` grava a linha em `PARCIAL` **antes** de chamar a CERC, justamente para garantir trilha de auditoria mesmo em falha (risco 5 do Plano 06). Mas `consultar_agenda` só fecha a consulta como `ERRO` no `except CercConsultaError:` — `CercTokenError` (de `services/cerc/token_provider.get_cerc_token`, uma exceção irmã, não subclasse de `CercConsultaError`) ou qualquer outra exceção que escape de `_chamar_cerc` (ex.: erro de banco dentro de `_registrar_requisicao`) não é capturada ali, deixando a linha permanentemente em `PARCIAL`. Não corrompe dado nem afeta outras consultas — é um gap isolado ao caminho de falha, recuperável (a correção é ampliar o `except` ou tornar `CercTokenError` subclasse de `CercConsultaError`) — mas real e ainda não corrigido; carregar para quem tocar `services/cerc/client.py` de novo (Plano 07/09 ou uma varredura futura de `consulta_agenda` presas em `PARCIAL` além do timeout de 15min do §5.5 sem terem passado por `COMPLETA_COM_TIMEOUT`).
