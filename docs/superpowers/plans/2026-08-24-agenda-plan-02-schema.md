...
- **The database already exists** — provisioned outside this plan (controller ran `gcloud sql databases create` + `gcloud sql users create` against the existing `app-db` instance), so this task does not create infrastructure, only applies schema to it:
  ...
  - Database: `agenda`. User: `agenda_app` — has `ALL PRIVILEGES` on the `agenda` database and `public` schema. **Correction (found by the final review of this plan):** `gcloud sql users create` granted `agenda_app` membership in `cloudsqlsuperuser` by default, which in turn had `CREATEDB`/`CREATEROLE` and `CREATE` on the `app` database (`ap-back-optin`'s tenant, same instance) — not "this database only" as originally stated here. The controller revoked `cloudsqlsuperuser` membership and `CREATEDB`/`CREATEROLE` from `agenda_app` directly on the instance (not part of any task's diff). Residual: `agenda_app` still has the Postgres-default `CONNECT` (not `CREATE`) on every database on the instance, including `app` — this is the out-of-the-box default for any role on any database and was not specifically revoked, since doing so touches `ap-back-optin`'s database and is out of this service's scope. Track as a shared-instance hardening item if/when someone owns cross-service Cloud SQL access policy for `app-db`.
